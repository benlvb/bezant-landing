...
The 1200×630 share card is stored as `landing/og-image.png.b64` and decoded to `landing/og-image.png` during the Vercel install step (and by `scripts/check-landing.mjs`). Re-render from the SVG with:

```sh
rsvg-convert -w 1200 -h 630 landing/og-card.svg -o landing/og-image.png
base64 -w 0 landing/og-image.png > landing/og-image.png.b64
```
